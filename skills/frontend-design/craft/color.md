...
> Adapted from the [Open Design](https://github.com/nexu-io/open-design) craft reference. Optimized for frontend-design skill.
...
|---|---|---|
| **Neutrals** | 70–90% | `--color-bg`, `--color-surface`, `--color-fg`, `--color-muted`, `--color-border` |
| **Accent** (one) | 5–10% | `--color-accent` only — never invent a second accent |
| **Semantic** | 0–5% | `--color-success`, `--color-warn`, `--color-danger` |
...
- **At most 2 visible uses of `--color-accent` per screen.** Typical pair:
  one eyebrow / chip + one primary CTA. Or one accent card + one tab
  pill. Pick a pair, not a flood.
- Links count as accent; demote to `--color-fg` underline if you also have a
...
- **Indigo `#6366f1`** (Tailwind `indigo-500`) is the most reliable
  AI-slop tell. The active `DESIGN.md` provides `--color-accent`; use it. If
